...
function runMigrations(): void {
  ...
  try {
    const columns = db.prepare("PRAGMA table_info(linked_reports)").all() as { name: string }[];
    const hasStarred = columns.some(col => col.name === 'starred');
    if (!hasStarred) {
      db.exec("ALTER TABLE linked_reports ADD COLUMN starred INTEGER DEFAULT 0");
  ...
  try {
    const columns = db.prepare("PRAGMA table_info(pbi_reports)").all() as { name: string }[];
    const hasStarred = columns.some(col => col.name === 'starred');
    if (!hasStarred) {
