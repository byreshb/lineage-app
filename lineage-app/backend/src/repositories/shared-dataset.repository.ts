...
export class SharedDatasetRepository {
  ...
  findById(id: number): SharedDataset | undefined {
    const row = this.db
      .prepare("SELECT * FROM shared_datasets WHERE id = ?")
      .get(id);
  ...
  findByName(name: string): SharedDataset | undefined {
    const row = this.db
      .prepare("SELECT * FROM shared_datasets WHERE dataset_name = ?")
      .get(name);
    return this.mapRow(row);
  }
  ...
  findByPath(path: string): SharedDataset | undefined {
    const row = this.db
      .prepare("SELECT * FROM shared_datasets WHERE dataset_path = ?")
      .get(path);
    return this.mapRow(row);
  }
  ...
  findAll(): SharedDataset[] {
    const rows = this.db
      .prepare("SELECT * FROM shared_datasets ORDER BY dataset_name")
      .all();
  ...
  count(): number {
    const row = this.db
      .prepare("SELECT COUNT(*) as count FROM shared_datasets")
      .get() as any;
    return row?.count || 0;
  }
