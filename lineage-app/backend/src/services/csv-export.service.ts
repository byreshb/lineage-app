...
export class CsvExportService {
  ...
  private getSsrsRowsForReport(
    ...
    for (const viewEdge of viewEdges) {
      ...
      const baseTables = this.findAllBaseTables(
        viewEdge.targetName,
        [],
        [],
        new Set(),
      ...
      for (const bt of baseTables) {
        ...
        let firstView =
          viewEdge.targetId > 0
            ? this.repos.view.findById(viewEdge.targetId)
            : null;
        if (!firstView)
          firstView = this.repos.view.findByName(viewEdge.targetName);
        const firstViewName = firstView
          ? this.formatNameWithSchema(firstView.viewName, firstView.schemaName)
          : viewEdge.targetName;
        ...
        let finalComment = bt.isExternal
          ? comment
            ? `${comment}; Base table not in SQL2(D300SQLDW01) metadata`
            : "Base table not in SQL2(D300SQLDW01) metadata"
          : comment;
    ...
    for (const procEdge of procEdges) {
      ...
      const { tables, procPath } = this.findTablesFromProc(
        procEdge.targetName,
        new Set(),
      );
      ...
      let firstProc =
        procEdge.targetId > 0
          ? this.repos.storedProc.findById(procEdge.targetId)
          : null;
      if (!firstProc)
        firstProc = this.repos.storedProc.findByName(procEdge.targetName);
      const firstProcName = firstProc
        ? this.formatNameWithSchema(firstProc.procName, firstProc.schemaName)
        : procEdge.targetName;
      ...
      for (const bt of tables) {
        ...
        const allProcs = [
          ...
          firstProcName,
          ...bt.procPath.filter(
            (p) => p.toLowerCase() !== firstProcName.toLowerCase(),
          ),
        ];
  ...
  private buildEntityChain(
    ...
    while (currentType && currentName) {
      ...
      if (currentType === "VIEW") {
        ...
        // (IDs can become stale after metadata reload)
        let proc =
          currentId > 0 ? this.repos.storedProc.findById(currentId) : null;
        if (!proc) proc = this.repos.storedProc.findByName(currentName);
        const displayName = proc
          ? this.formatNameWithSchema(proc.procName, proc.schemaName)
          : currentName;
  ...
  private findAllBaseTables(
    ...
    for (const ref of tableRefs) {
      ...
      if (nestedView) {
        ...
        const deeper = this.findAllBaseTables(
          nestedFullName,
          newViewPath,
          currentProcPath,
          visited,
  ...
  private findTablesFromProc(
    ...
    for (const ref of tableRefs) {
      ...
      if (nestedView) {
        ...
        const viewTables = this.findAllBaseTables(
          viewFullName,
          [viewFullName],
          procPath,
          visited,
   ...
   */
  private getCustomTablesFromReport(
    reportId: number,
  ): Array<{ schema: string; tableName: string }> {
    const edges = this.repos.lineage.findByReportId(reportId);
    ...
    for (const procEdge of procEdges) {
      if (!procEdge.targetName) continue;
      const { tables: procTables } = this.findTablesFromProc(
        procEdge.targetName,
        new Set(),
      );
   ...
   */
  private getCustomTablesFromPbiReport(
    reportId: number,
  ): Array<{ schema: string; tableName: string }> {
    ...
    for (const pbiTable of pbiTables) {
      ...
      if (directTable && this.isCustomTable(directTable.tableName)) {
        const key =
          `${directTable.schemaName}.${directTable.tableName}`.toLowerCase();
        if (!seen.has(key)) {
          seen.add(key);
          tables.push({
            schema: directTable.schemaName || "",
  ...
  exportCustomTablesByReport(
    ...
    const header =
      ["Report Name", "Schema", "Custom Table", "Type", "Report Path"].join(
        ",",
      ) + "\n";
    let csv = header;
    ...
    if (scope === "SSRS" || scope === "Both") {
      ...
      if (starredOnly) {
        ...
        for (const linkedReport of linkedReports) {
          ...
          if (templateReport && templateReport.status === "COMPLETED") {
            const customTables = this.getCustomTablesFromReport(
              templateReport.id!,
            );
            for (const table of customTables) {
              customTablesByReport.push({
                reportName: linkedReport.linkedReportName,
                schema: table.schema,
                customTable: table.tableName,
                reportType: "SSRS",
                reportPath: linkedReport.linkedReportPath || "",
              });
            }
          }
        }
  ...
  private generateUniqueColumnRows(
    ...
    for (const table of tables) {
      ...
      let sql2Columns = this.repos.column.findSql2ColumnsByTable(
        table.tableName,
        table.schema,
      );
      ...
      let trn1Columns = this.repos.column.findTrn1ColumnsByObject(
        table.tableName,
        table.schema,
      );
      if (trn1Columns.length === 0) {
        trn1Columns = this.repos.column.findTrn1ColumnsByObject(
          table.tableName,
        );
      }

      // Build a map of TRN1 columns by name for matching
      const trn1ByName = new Map(
        trn1Columns.map((c) => [c.columnName.toLowerCase(), c]),
      );
        ...
        for (const sql2Col of sql2Columns) {
          ...
          rows +=
            [
              ...
              trn1Col
                ? trn1Col.isNullable === true
                  ? "Yes"
                  : trn1Col.isNullable === false
                    ? "No"
                    : "-"
                : "-",
              // SQL2 columns
              ...
              sql2Col.isNullable === true
                ? "Yes"
                : sql2Col.isNullable === false
                  ? "No"
                  : "-",
            ].join(",") + "\n";
        }
   ...
   */
  private getUniqueTablesFromPbiReport(
    reportId: number,
  ): Array<{ schema: string; tableName: string }> {
    const pbiTables = this.repos.pbiTable.findByReportId(reportId);
    const tables: Array<{ schema: string; tableName: string }> = [];
