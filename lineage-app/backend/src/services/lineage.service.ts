...
import {
  LineageGraphDto, LineageNodeDto, LineageEdgeDto, ParsingWarningDto,
  SourceTableDto, LineageEdge, Dataset, NodeType, Relationship, DiscoveryMethod
} from '../types/index.js';
import { extractTables, extractStoredProcName, detectDynamicSqlWarnings, extractProcedureCalls, TableReference } from '../parsers/sql.analyzer.js';
import dayjs from 'dayjs';

const FALSE_POSITIVES = new Set([
  'SYSPR', 'SYSPRO', 'DB', 'DBO', 'SYSP', 'SYS',
  'MASTER', 'TEMPDB', 'MSDB', 'MODEL',
  'INFORMATION_SCHEMA', 'SYSCUSTOMIZATIONS',
  'SUN300', 'SUN300D', 'SUN300DSYSSQL01',
  'SYSPROCOMPANYC', 'SYSPROCOMPANYS',
  'SYSPROREPORTING', 'SYSPROEPASAGE',
...
export class LineageService {
  ...
  private loadLinkedServers(): void {
    ...
    for (const ls of linkedServers) {
      if (ls.alias) {
        this.linkedServerMap.set(ls.alias.toUpperCase(), ls.actualServer || 'UNKNOWN');
      }
    }
  ...
  private resolveLinkedServer(serverAlias: string): { isKnown: boolean; actualServer: string } {
  ...
  buildLineage(reportId: number): void {
    ...
    for (const dataset of datasets) {
      this.saveEdge(reportId, 'REPORT', reportId, report.reportName || '',
        'DATASET', dataset.id!, dataset.datasetName, 'CONTAINS');
  ...
  private processStoredProcedure(reportId: number, dataset: Dataset, commandText: string | null): void {
    ...
    if (proc) {
      this.saveEdge(reportId, 'DATASET', dataset.id!, dataset.datasetName,
        'PROC', proc.id!, proc.procName, 'CALLS');

      if (proc.definition) {
        // Analyze tables referenced by this proc
        this.analyzeAndLinkTables(reportId, 'PROC', proc.id!, proc.procName, proc.definition);
        // Analyze nested proc calls (proc-to-proc chains)
        this.analyzeAndLinkProcs(reportId, 'PROC', proc.id!, proc.procName, proc.definition, new Set());
      }
    } else {
      console.warn(`Stored procedure not found in metadata: ${procName}`);
      this.saveEdge(reportId, 'DATASET', dataset.id!, dataset.datasetName,
        'PROC_NOT_FOUND' as NodeType, -1, procName, 'CALLS');
    }
  }

  private processDirectSql(reportId: number, dataset: Dataset, sql: string | null): void {
    if (!sql) return;
    this.analyzeAndLinkTables(reportId, 'DATASET', dataset.id!, dataset.datasetName, sql);
  }
  ...
  private processSharedDataSet(reportId: number, dataset: Dataset): void {
    ...
    if (shared) {
      this.saveEdge(reportId, 'DATASET', dataset.id!, dataset.datasetName,
        'SHARED_DATASET', shared.id!, shared.datasetName, 'USES');

      if (shared.commandText) {
        this.analyzeAndLinkTables(reportId, 'SHARED_DATASET', shared.id!, shared.datasetName, shared.commandText);
      }
    } else {
      console.warn(`SharedDataset not found in metadata: ${sharedName}`);
      this.saveEdge(reportId, 'DATASET', dataset.id!, dataset.datasetName,
        'SHARED_DATASET_NOT_FOUND' as NodeType, -1, sharedName, 'USES');
    }
  }

  private analyzeAndLinkTables(
    reportId: number, sourceType: string, sourceId: number,
    sourceName: string, sql: string, visitedViews: Set<string> = new Set()
  ): void {
    ...
    const sqlServerDeps = this.repos.procDependency.findByObjectName(sourceName);
    const sqlServerDepNames = new Set(sqlServerDeps.map(d => d.dependsOnName?.toLowerCase()).filter(Boolean) as string[]);
      ...
      if (ref.sourceType === 'DYNAMIC') {
        ...
        if (!this.isFalsePositive(ref.tableName)) {
          ...
          this.saveEdge(reportId, sourceType, sourceId, sourceName,
            'TABLE_NOT_FOUND' as NodeType, -1, dynamicName, 'READS_FROM', discoveryMethod);
        }
        ...
        const actualServer = linkedInfo.isKnown ? linkedInfo.actualServer : 'UNKNOWN';
        ...
        if (!this.isFalsePositive(ref.tableName)) {
          this.saveEdge(reportId, sourceType, sourceId, sourceName,
            'TABLE_NOT_FOUND' as NodeType, -1, fullName, 'READS_FROM', discoveryMethod);
        }
        ...
        if (view) {
          ...
          if (table) {
            this.saveEdge(reportId, sourceType, sourceId, sourceName,
              'TABLE', table.id!, table.tableName, 'READS_FROM', discoveryMethod);
          } else if (!this.isFalsePositive(ref.tableName)) {
            const fullName = [ref.server, ref.database, ref.schema, ref.tableName].filter(Boolean).join('.');
            this.saveEdge(reportId, sourceType, sourceId, sourceName,
              'TABLE_NOT_FOUND' as NodeType, -1, fullName, 'READS_FROM', discoveryMethod);
          }
        }
    ...
    for (const dep of sqlServerDeps) {
      ...
      if (depType === 'VIEW') {
        ...
        if (view) {
          ...
          this.saveEdge(reportId, sourceType, sourceId, sourceName,
            'VIEW_NOT_FOUND' as NodeType, -1, dep.dependsOnName, 'READS_FROM', 'SQL_SERVER');
        }
      } else {
        const table = this.repos.table.findByName(dep.dependsOnName);
        if (table) {
          this.saveEdge(reportId, sourceType, sourceId, sourceName,
            'TABLE', table.id!, table.tableName, 'READS_FROM', 'SQL_SERVER');
        } else {
          const fullName = dep.dependsOnSchema
            ? `${dep.dependsOnSchema}.${dep.dependsOnName}`
            : dep.dependsOnName;
          this.saveEdge(reportId, sourceType, sourceId, sourceName,
            'TABLE_NOT_FOUND' as NodeType, -1, fullName, 'READS_FROM', 'SQL_SERVER');
        }
      }
  ...
  private analyzeAndLinkProcs(
    reportId: number, sourceType: string, sourceId: number,
    sourceName: string, sql: string, visitedProcs: Set<string>, depth: number = 1
  ): void {
    ...
    for (const procCall of procCalls) {
      ...
      if (!calledProc && procCall.schema) {
        ...
        calledProc = this.repos.storedProc.findByName(`${procCall.schema}.${procCall.procName}`);
      }

      if (calledProc) {
        // Save PROC -> PROC edge
        this.saveEdge(reportId, sourceType, sourceId, sourceName,
          'PROC', calledProc.id!, calledProc.procName, 'CALLS', 'REGEX');

        // Recursively analyze the called proc
        if (calledProc.definition) {
          // Analyze tables referenced by this nested proc
          this.analyzeAndLinkTables(reportId, 'PROC', calledProc.id!, calledProc.procName, calledProc.definition);
          // Continue recursion for proc-to-proc chains
          this.analyzeAndLinkProcs(reportId, 'PROC', calledProc.id!, calledProc.procName, calledProc.definition, visitedProcs, depth + 1);
        }
      } else {
        // Proc not found - save as PROC_NOT_FOUND
        const fullName = procCall.schema ? `${procCall.schema}.${procCall.procName}` : procCall.procName;
        this.saveEdge(reportId, sourceType, sourceId, sourceName,
          'PROC_NOT_FOUND' as NodeType, -1, fullName, 'CALLS', 'REGEX');
      }
    }
  }
  ...
  private findTableByNameOnly(tableName: string, schema: string | null) {
    ...
    if (schema) {
      const withSchema = this.repos.table.findBySchemaAndName(schema, tableName);
  ...
  private saveEdge(
    reportId: number, sourceType: string, sourceId: number, sourceName: string,
    targetType: string, targetId: number, targetName: string, relationship: string,
  ...
  getLineageGraph(reportId: number): LineageGraphDto {
    ...
    for (const edge of edges) {
      ...
      if (edge.targetType === 'PROC_NOT_FOUND') {
        warnings.push({ entityType: 'PROC', entityName: edge.targetName || '', warning: 'Stored procedure not found in metadata' });
      } else if (edge.targetType === 'SHARED_DATASET_NOT_FOUND') {
        warnings.push({ entityType: 'SHARED_DATASET', entityName: edge.targetName || '', warning: 'SharedDataset not found in metadata' });
      } else if (edge.targetType === 'TABLE_NOT_FOUND') {
        warnings.push({ entityType: 'TABLE', entityName: edge.targetName || '', warning: 'Table not found in metadata' });
      }
    }
      ...
      if (proc?.definition) {
        for (const w of detectDynamicSqlWarnings(proc.definition)) {
          ...
          if (!seenWarnings.has(key)) {
            ...
            warnings.push({ entityType: 'PROC', entityName: proc.procName, warning: w });
          }
        }
      }
    }
    ...
    for (const viewId of viewIds) {
      ...
      if (view?.definition) {
        for (const w of detectDynamicSqlWarnings(view.definition)) {
          ...
          if (!seenWarnings.has(key)) {
            seenWarnings.add(key);
            warnings.push({ entityType: 'VIEW', entityName: view.viewName, warning: w });
          }
        }
      }
    }
    ...
    return {
      ...
      lastAnalyzed: report.lastRunAt ? dayjs(report.lastRunAt).format('MMM D, YYYY h:mm A') : null,
  ...
  private buildNodeKey(type: string, id: number | null, name: string | null): string {
    if (!id || id <= 0) {
  ...
  private buildNode(type: string, id: number | null, name: string | null): LineageNodeDto {
    ...
    if (['PROC_NOT_FOUND', 'SHARED_DATASET_NOT_FOUND', 'TABLE_NOT_FOUND', 'VIEW_NOT_FOUND'].includes(type)) {
    ...
    if (name && name.startsWith('[') && name.includes('->')) {
      ...
      if (linkedMatch) {
        const [, alias, actualServer, database, schema, tableName] = linkedMatch;
  ...
  getSourceTables(reportId: number): SourceTableDto[] {
    ...
    for (const edge of tableEdges) {
      ...
      const key = edge.targetId > 0 ? `id:${edge.targetId}` : `name:${edge.targetName}`;
  ...
  private isFalsePositive(tableName: string): boolean {
    ...
    if (tableName.length <= 8 && tableName === tableName.toUpperCase()) {
      if (!/(TBL|TABLE|LOG|HIST|DATA|INFO|DETAIL|HDR|DTL)$/i.test(upper)) return true;
    }
    return false;
  }
  ...
  private buildChain(edges: LineageEdge[], datasetMap: Map<number | null, Dataset>, tableEdge: LineageEdge) {
    // Separate arrays for procs and views
    const chain = { datasetName: '', datasetType: '', procs: [] as string[], views: [] as string[], comment: '' };
