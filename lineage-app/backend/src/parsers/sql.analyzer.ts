...
const SQL_KEYWORDS = new Set([
  ...
  'TOP', 'AS', 'ON', 'WITH', 'INTO', 'VALUES', 'SET',
  'INSERT', 'UPDATE', 'DELETE', 'MERGE', 'WHEN', 'THEN', 'ELSE',
  'CASE', 'END', 'NULL', 'IS', 'LIKE', 'BETWEEN', 'JOIN',
  'BEGIN', 'DECLARE', 'IF', 'WHILE', 'RETURN', 'NOLOCK',
  'OVER', 'PARTITION', 'ROW_NUMBER', 'RANK', 'DENSE_RANK',
  'NVARCHAR', 'VARCHAR', 'INT', 'INTEGER', 'DECIMAL', 'FLOAT',
  'CHAR', 'TEXT', 'BIT', 'DATE', 'DATETIME', 'TIMESTAMP',
  'TRUE', 'FALSE', 'CAST', 'CONVERT', 'COALESCE', 'ISNULL',
  'MAX', 'MIN', 'SUM', 'COUNT', 'AVG', 'LEN', 'RTRIM', 'LTRIM',
]);
...
function extractDynamicTables(sql: string): TableReference[] {
  ...
  for (const pattern of DYNAMIC_TABLE_PATTERNS) {
    ...
    while ((match = regex.exec(sql)) !== null) {
      ...
      if (['dbo', 'sys', 'syspro', 'stage', 'bi', 'xref'].includes(schema.toLowerCase()) === false) {
...
function extractFourPartTables(sql: string): TableReference[] {
  ...
  while ((match = pattern.exec(cleanedSql)) !== null) {
    ...
    if (isKeyword(server) || isKeyword(database) || isKeyword(schema) || isKeyword(table)) {
      continue;
    }
...
function extractTablesWithParser(sql: string): TableReference[] {
  ...
  try {
    ...
    const tableList = sqlParser.tableList(cleanedSql, { database: 'transactsql' });
...
export function extractTables(sql: string | null): TableReference[] {
  ...
  for (const ref of parserResults) {
    ...
    if (!tableMap.has(key) && !foundTableNames.has(ref.tableName.toLowerCase())) {
      tableMap.set(key, ref);
      foundTableNames.add(ref.tableName.toLowerCase());
    }
  }
...
export function extractStoredProcName(commandText: string | null): string | null {
...
export function detectDynamicSqlWarnings(sql: string | null): string[] {
  ...
  if (upperSql.includes('SP_EXECUTESQL')) {
    warnings.push('Contains sp_executesql - dynamic SQL table references may be missing');
  }

  if (/EXEC(UTE)?\s*\(\s*@\w+/i.test(sql)) {
    warnings.push('Contains EXEC(@variable) - dynamic SQL table references may be missing');
  }

  if (/@\w+\s*\+\s*'\.'\s*\+\s*@\w+/i.test(sql)) {
    warnings.push('Contains dynamic table name construction - table references may be missing');
  }

  if (upperSql.includes('OPENQUERY')) {
    warnings.push('Contains OPENQUERY - remote table references may not be captured');
  }

  if (upperSql.includes('OPENROWSET')) {
    warnings.push('Contains OPENROWSET - external data source references may not be captured');
  }
...
const SYSTEM_PROCS = new Set([
  'SP_EXECUTESQL', 'SP_EXECUTE', 'SP_PREPEXEC', 'SP_CURSOREXECUTE',
  'SP_CURSOR', 'SP_CURSOROPEN', 'SP_CURSORCLOSE', 'SP_CURSORFETCH',
  'SP_CURSOROPTION', 'SP_CURSORPREPARE', 'SP_CURSORUNPREPARE',
  'SP_DESCRIBE_CURSOR', 'SP_DESCRIBE_CURSOR_COLUMNS', 'SP_DESCRIBE_CURSOR_TABLES',
  'SP_REFRESHSQLMODULE', 'SP_REFRESHVIEW',
  'SP_ADDMESSAGE', 'SP_DROPMESSAGE',
  'SP_SEND_DBMAIL', 'SP_START_JOB', 'SP_STOP_JOB',
  'SP_HELP', 'SP_HELPTEXT', 'SP_HELPINDEX', 'SP_HELPCONSTRAINT',
  'SP_DEPENDS', 'SP_TABLES', 'SP_COLUMNS', 'SP_STORED_PROCEDURES',
  'SP_RENAME', 'SP_RENAMEDB', 'SP_RECOMPILE',
  'SP_GETAPPLOCK', 'SP_RELEASEAPPLOCK',
  'SP_XML_PREPAREDOCUMENT', 'SP_XML_REMOVEDOCUMENT',
  'XP_CMDSHELL', 'XP_FILEEXIST', 'XP_LOGEVENT', 'XP_SENDMAIL',
...
const EXEC_PROC_PATTERN = /(?:EXEC(?:UTE)?)\s+(?:(?:\[?(\w+)\]?)\s*\.\s*)?(?:\[?(\w+)\]?)(?:\s|;|$|@)/gi;
