...
export function parseRdlContent(content: string, reportName: string): RdlParseResult {
  const contentLength = content?.length || 0;
  const endsWithReport = content?.trim().endsWith('</Report>') || false;
  const last50Chars = content?.slice(-50) || '';

  console.log(`Parsing RDL content for: ${reportName} (${contentLength} chars, valid XML ending: ${endsWithReport})`);

  if (!endsWithReport) {
    console.warn(`WARNING: RDL content appears truncated for ${reportName}. Last 50 chars: "${last50Chars}"`);
  }
  ...
  try {
    ...
    console.error(`XML PARSE ERROR for ${reportName}: ${err instanceof Error ? err.message : err}`);
    console.error(`Content length: ${contentLength}, ends with </Report>: ${endsWithReport}`);
    throw err;
  }
  ...
  console.log(`Parsed ${result.dataSources.length} data sources, ${result.datasets.length} datasets from ${reportName}`);
  return result;
}
...
function extractReportName(rdlFilePath: string): string {
  ...
  return path.basename(rdlFilePath)
    .replace('.rdl', '')
    .replace(' (Template)', '')
    .replace('(Template)', '')
    .trim();
...
function parseDataSource(dsElement: any): Omit<DataSource, 'id' | 'reportId'> | null {
  ...
  if (reference) {
    ...
    const connectionString = dsElement.ConnectionProperties?.ConnectString || dsElement.ConnectionString;
    if (connectionString) {
      ds.connectionString = connectionString;
      ds.sourceType = 'EMBEDDED';
      parseConnectionString(ds, connectionString);
      console.log(`Found embedded DataSource: ${ds.sourceName} -> ${connectionString.substring(0, 50)}...`);
    }
  }
...
function parseConnectionString(ds: Omit<DataSource, 'id' | 'reportId'>, connStr: string): void {
  if (!connStr) return;
...
function parseDataSet(dataSetElement: any): Omit<Dataset, 'id' | 'reportId'> | null {
  ...
  console.log(`Dataset: ${dataset.datasetName} - Type: ${commandType} - Command: ${commandText ? commandText.substring(0, 50) + '...' : 'null'}`);
