...
export async function pbiRoutes(
  ...
  app.get("/reports/:id/export", async (request, reply) => {
    ...
    try {
      ...
      const report = repos.pbiReport.findById(parseInt(id, 10));
      const fileName =
        report?.reportName?.replace(/[^a-zA-Z0-9]/g, "_") || `pbi_report_${id}`;

      reply.header(
        "Content-Disposition",
        `attachment; filename="lineage_pbi_${fileName}.csv"`,
      );
      reply.type("text/csv");
      return csv;
    } catch (error) {
  ...
  app.get("/export-all", async (request, reply) => {
    ...
    reply.header(
      "Content-Disposition",
      'attachment; filename="lineage_pbi_all_reports.csv"',
    );
    reply.type("text/csv");
    return csv;
  });
  ...
  app.get("/export-starred", async (request, reply) => {
    ...
    reply.header(
      "Content-Disposition",
      'attachment; filename="lineage_pbi_starred_reports.csv"',
    );
    reply.type("text/csv");
    return csv;
  });
  ...
  app.get("/export-all-html", async (request, reply) => {
    try {
      ...
      reply.header(
        "Content-Disposition",
        'attachment; filename="lineage_pbi_all_reports.html"',
      );
  ...
  app.get("/export-starred-html", async (request, reply) => {
    try {
      ...
      reply.header(
        "Content-Disposition",
        'attachment; filename="lineage_pbi_starred_reports.html"',
      );
      reply.type("text/html");
      return html;
    } catch (error: any) {
